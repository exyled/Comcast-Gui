...
import sys
import os
from subprocess import Popen #Works on this end, as long as the file is in the same directory
import webbrowser
...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button = tk.Button(self, text="0. Script",
                            command=lambda: os.startfile("TopLevelMenu.py")) #This just opens the script in a txt editor
        button.pack()
        button0 = tk.Button(self, text="0. Script",
                            command = lambda: Popen('python ScratchPad.py')) #Works
        button0.pack()
                    #os.system('python TopLevelMenu.py')
                    #subprocess('python TopLevelMenu') #Doesn't Work
                            #command = lambda: os.startfile("TopLevelMenu.py"))  # This just opens the script in a txt editor
                    #import file
                    #import TopLevelMenu
                            #command = lambda: import (TopLevelMenu) #This just opens the script in a txt editor
                    #import ScratchPad
                            #command=lambda: exec(ScratchPad.py)) #Doesn't Work
                            #command=lambda: open('python ScratchPad.py')) #This just opens the script in a txt editor
